...
#
# Builds fuzz_ogg_demux with clang's source-based coverage instrumentation,
# replays the corpus through it once (libFuzzer -runs=0), and prints a
# per-function coverage report for the demuxer. Pass --html to additionally
...
# clang++, llvm-profdata, and llvm-cov).
...
for spec in "clang++:$clangxx" "llvm-profdata:$profdata" "llvm-cov:$llvmcov"; do
    name="${spec%%:*}"
    path="${spec#*:}"
...
# Coverage-only build dir (kept separate from build-libfuzzer/ so the normal
# fuzzing build doesn't pick up the instrumentation overhead).
...
echo "[cov] building fuzz_ogg_demux"
cmake --build build-cov --target fuzz_ogg_demux >/dev/null

binary="build-cov/fuzz_ogg_demux"
...
# Report on the demuxer source only; the harness itself is excluded. Override
# with IGNORE_RE to widen or narrow.
